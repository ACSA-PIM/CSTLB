...
config=/root/codes/Victima/sniper/config/UniNDP/CPU/baseline_CPU_1core_no_translation.cfg
...
run_and_notify() {
    local dir=$1
    (cd "$dir" && $sniper --power -c $config -s stop-by-icount:500000000 --genstats --traces=/root/codes/Victima/traces/$dir.sift && echo "$dir completed") &
}
