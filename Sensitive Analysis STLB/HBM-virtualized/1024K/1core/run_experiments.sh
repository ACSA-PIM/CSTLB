...
config=/root/codes/Victima/sniper/config/UniNDP/CPU-HBM/baseline_CPU_1core_cuckoo_potm_channel_1024K.cfg
...
run_and_notify() {
    ...
    (cd "$dir" && $sniper -c $config -s stop-by-icount:500000000 --genstats -g --perf_model/ptw/enabled=false -g --perf_model/ptw/virtualized=true --traces=/root/codes/Victima/traces/$dir.sift && echo "$dir completed") &
}
